...
type TaskSpec struct {
	...
	// +kubebuilder:validation:Minimum=0
	// +kubebuilder:validation:XValidation:rule="self.minAvailable == null || self.minAvailable <= self.replicas",message="minAvailable must not be greater than replicas"
	// +optional
	...
	// +kubebuilder:validation:XValidation:rule="self.partitionPolicy == null || self.partitionPolicy.totalPartitions * self.partitionPolicy.partitionSize == self.replicas",message="The product of totalPartitions and partitionSize must equal replicas"
	// +optional
